...
ROW_FILES = {
    ...
    'C': r"D:\2026\WPF_Sudoku\Sudoku_256\C第三行符阖排列_補P10R.xlsx",
...
def solve_evolution(initial, target_row, permutations, row_idx):
    ...
    for c_idx, val in enumerate(target_perm):
        model.Add(x[(target_row_idx, c_idx)] == val)
    ...
    target_perm_tuple = tuple(target_perm)
    if target_perm_tuple not in permutation_set:
        return {'status': 'ERROR', 'message': f'终局排列不在{target_row}行排列集合中', 'elapsed': time.time() - start_time}
    
    # 使用AddAllowedAssignments添加排列约束
    model.AddAllowedAssignments([x[(target_row_idx, c)] for c in range(16)], list(permutation_set))
    ...
    for row_letter in 'ABCDEFGHIJKLMNOP':
        ...
        try:
            ...
            if result.get('status') in ['OPTIMAL', 'FEASIBLE'] and result.get('solution'):
                ...
                for r, row_vals in sol.items():
                    marker = '*' if r == row_letter else ' '
                    print(f"    {marker}Row{r}: " + " ".join(f"{v:3d}" for v in row_vals))
