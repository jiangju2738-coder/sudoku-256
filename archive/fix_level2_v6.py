#!/usr/bin/env python3
"""
修复 Level 2 CNF v6 — 修正 AMO 循环范围 + 重新生成 Row 16
"""
...
n_vars = s_off[15] + counts[15] - 1  # 2227053

# 计算截断点
fixed = 16*16*120*3 + 256*121 + 92
rows_1_15 = sum((4*c-1)+16*c for c in counts[:15])
truncate_to = fixed + rows_1_15  # 22321853

print(f"truncate_to = {truncate_to:,}")
print(f"Current lines: ", end="", flush=True)
...
print(f"  ALO: 1, AMO: {4*n16-6}, Assoc: {16*n16}, Total Row16: {1+(4*n16-6)+16*n16:,}", flush=True)
...
# AMO (修正: k 从 0 到 n16-2, 共 n16-1 个 S 变量)
print("  [2/3] AMO (corrected)...", flush=True)
for k in range(n16 - 1):  # FIX: was range(n16)
    pk = p0 + k
    sk = s0 + k
    if k == 0:
        # S[0] <-> P[0]
...
# 预期: fixed + rows_1_15 + ALO + AMO + Assoc
# AMO = 2 + 4*(n16-2) = 4*n16 - 6
expected = fixed + rows_1_15 + 1 + (4*n16-6) + 16*n16
print(f"\nExpected: {expected:,}", flush=True)
print(f"Actual:   {final:,}", flush=True)
