...
class Sudoku256Verifier:
    ...
    def __init__(self, known_digits, row_perms):
        ...
        # 每行已使用的符闔排列索引
        self.row_selections = [-1] * self.N
        
        # 列和宮的數字使用情況
        self.col_used = [[False] * self.N for _ in range(self.N)]
        self.box_used = [[False] * self.N for _ in range(self.N)]
...
def validate_solution(grid, known_map, row_perms):
    ...
    # 3. 檢查列約束
    for c in range(self.N):
        col_vals = [grid[r][c] for r in range(self.N)]
