...
print(f"n_vars={n_vars:,}, total_p={total_p:,}")

# 检查当前状态
r = subprocess.run(['wc','-l',OUT], capture_output=True, text=True)
cur_lines = int(r.stdout.split()[0])
cur_size = os.path.getsize(OUT)
print(f"Current: {cur_lines:,} lines, {cur_size/1024/1024:.1f} MB")

# 计算截断点: 固定部分 + Rows 1-15 ExactlyOne+AMO+Association
fixed = 16*16*120*3 + 256*121 + 92
rows_1_15 = sum((4*c-1)+16*c for c in counts[:15])  # 4n-1 per row
truncate_to = fixed + rows_1_15
print(f"Truncate to: {truncate_to:,} (Row 16 starts at {truncate_to+1})")
...
with open(OUT,'w') as f:
    f.writelines(hdr)
print(f"  Truncated to {os.path.getsize(OUT)/1024/1024:.1f} MB")
...
s0 = s_off[15]
print(f"Row 16: n={n16}, P=[{p0},{p0+n16-1}], S=[{s0},{s0+n16-2}]")
print(f"  ExactlyOne: {4*n16-1:,} clauses")
print(f"  Association: {16*n16:,} clauses")
...
for k in range(n16):
    ...
    if k%200000==0 and k>0:
        print(f"    k={k:,}/{n16:,}", flush=True)
print("  AMO done", flush=True)

# Association
print("  [3/3] Association...", flush=True)
for j in range(n16):
    pj = p0 + j
    perm = p16[j]
    for c in range(16):
        W(f"-{pj} {X0+15*256+c*16+perm[c]} 0")
    if j%10000==0 and j>0:
        print(f"    j={j:,}/{n16:,} ({j/n16*100:.1f}%)", flush=True)
...
exp_clauses = fixed + rows_1_15 + (4*n16-1) + 16*n16
print(f"\nExpected: {exp_clauses:,} clauses")
print(f"Actual:   {final:,} lines")
...
for i in range(len(lines)):
    if lines[i].startswith('p cnf'):
        ...
        print(f"\nHeader: p cnf {n_vars} {final}")
        break
with open(OUT,'w') as f: f.writelines(lines)

final_sz = os.path.getsize(OUT)
print(f"✅ Level 2 CNF 完成!")
print(f"   {OUT}")
print(f"   Size: {final_sz/1024/1024/1024:.2f} GB")
print(f"   Vars: {n_vars:,}")
print(f"   Clauses: {final:,}")
