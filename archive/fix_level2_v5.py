...
print(f"Current: {cur:,} lines, {os.path.getsize(OUT)/1024/1024:.1f} MB", flush=True)

# 精确计算截断点: 固定 + Rows 1-15 (ExactlyOne 4n-1 + AMO + Assoc)
# ExactlyOne(n) = 1(ALO) + 2(k=0) + 4*(n-1)(k>=1) = 4n-1
fixed = 16*16*120*3 + 256*121 + 92
rows_1_15 = sum((4*c-1)+16*c for c in counts[:15])
truncate_to = fixed + rows_1_15
print(f"Truncate to line: {truncate_to:,}", flush=True)
...
with open(OUT,'w') as f:
    f.writelines(hdr)
print(f"  After truncate: {os.path.getsize(OUT)/1024/1024:.1f} MB", flush=True)
...
print(f"  ExactlyOne: {4*n16-1:,}, AMO: {4*n16-4:,}, Assoc: {16*n16:,}", flush=True)
...
print("  [1/3] ALO done", flush=True)

# 2. Sequential Counter AMO
...
fixed_total = fixed + rows_1_15 + (4*n16-1) + 16*n16
print(f"\nExpected total: {fixed_total:,}", flush=True)
...
for i in range(len(lines)):
    if lines[i].startswith('p cnf'):
        ...
        print(f"\nHeader updated: p cnf {n_vars} {final}", flush=True)
        break
with open(OUT,'w') as f: f.writelines(lines)

final_sz = os.path.getsize(OUT)
print(f"\n{'='*60}", flush=True)
print(f"✅ Level 2 CNF 生成完成!", flush=True)
print(f"   文件: {OUT}", flush=True)
