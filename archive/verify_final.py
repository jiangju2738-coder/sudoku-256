...
print(f"  {'行':>4} {'已知':>6} {'目标':>6} {'剩余':>6} {'符阖一致':>8}")
for r in range(1, 17):
    known = row_count.get(r, 0)
    remaining = 16 - known
    perms = config.get('fuhh_permutations', {}).get(r, [])
    fuhh_ok = '✓' if perms else '✗'
    print(f"  {r:>4} {known:>6} {target_dist[r-1]:>6} {remaining:>6} {fuhh_ok:>8}")
