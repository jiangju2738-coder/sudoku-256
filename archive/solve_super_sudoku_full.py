...
def load_all_row_constraints(base_dir: str, sample_size: int = 5000) -> Dict[int, np.ndarray]:
    ...
    try:
        ...
        for line in lines:
            ...
            if len(values) == 16 and row_idx < 16:
                grid[row_idx, :] = values
                row_idx += 1
                print(f"  第{row_idx}行: {values[:5]}...{values[-5:]}")
...
class SuperSudoku16x16Solver:
    ...
    def __init__(self, row_constraints: Dict[int, np.ndarray], initial_grid: np.ndarray):
        ...
        # 預計算每格的初始可能值
        self.cell_possibilities: Dict[Tuple[int, int], Set[int]] = {}
    ...
    def _init_possibilities(self):
        ...
        for row in range(self.GRID_SIZE):
            for col in range(self.GRID_SIZE):
                self.cell_possibilities[(row, col)] = self._get_possibilities(row, col)
    
    def _get_possibilities(self, row: int, col: int) -> Set[int]:
        """計算單元格的可能值"""
        possibilities = set(range(1, 17))
        
        # 同行
        possibilities -= set(self.grid[row, :])
        possibilities -= {0}
        
        # 同列
        possibilities -= set(self.grid[:, col])
        ...
        for i in range(box_row * self.BOX_SIZE, (box_row + 1) * self.BOX_SIZE):
            for j in range(box_col * self.BOX_SIZE, (box_col + 1) * self.BOX_SIZE):
                if self.grid[i, j] != 0:
                    possibilities.discard(self.grid[i, j])
        
        return possibilities
    
    def _update_possibilities(self, row: int, col: int):
        """更新相關格子的可能值"""
        # 更新同行
        for c in range(self.GRID_SIZE):
            if (row, c) in self.cell_possibilities:
                self.cell_possibilities[(row, c)] = self._get_possibilities(row, c)
        
        # 更新同列
        for r in range(self.GRID_SIZE):
            if (r, col) in self.cell_possibilities:
                self.cell_possibilities[(r, col)] = self._get_possibilities(r, col)
        ...
        for i in range(box_row * self.BOX_SIZE, (box_row + 1) * self.BOX_SIZE):
            for j in range(box_col * self.BOX_SIZE, (box_col + 1) * self.BOX_SIZE):
                if (i, j) in self.cell_possibilities:
                    self.cell_possibilities[(i, j)] = self._get_possibilities(i, j)
        ...
        for row in range(self.GRID_SIZE):
            for col in range(self.GRID_SIZE):
                if self.grid[row, col] == 0:
                    poss = self.cell_possibilities.get((row, col), set())
    ...
    def generate_visualization(self, output_path: str, all_constraints: Dict[int, np.ndarray]):
        """生成可視化 HTML"""
...
<body>
    <div class="container">
        ...
        <p class="subtitle">五維思維框架 (點-線-面-體-球-時空) | MRV 回溯優化 | 16 行符闔排列約束</p>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{sum(len(v) for v in all_constraints.values())}</div>
...
'''
        ...
        if self.solutions:
            ...
            for i in range(16):
                for j in range(16):
                    ...
                    html_content += f'<div class="{cell_class}">{val}</div>'
        else:
        ...
        <div class="solution-section">
            <h2 style="color: #333; margin-bottom: 15px;">📋 行約束規則總覽</h2>
            <div class="row-constraints">
'''
        
        # 顯示所有行的約束
        for row_idx in range(1, 17):
            if row_idx in all_constraints and len(all_constraints[row_idx]) > 0:
                perms = all_constraints[row_idx]
                html_content += f'''
                <div class="row-constraint">
                    <h4>第{row_idx}行 - {len(perms)} 個排列模式</h4>
                    <div class="perm">範例: {", ".join(map(str, perms[0]))}</div>
                    <div style="color: #666; font-size: 12px;">每行從這些排列中選擇，需滿足列不衝突</div>
                </div>
'''
...
'''.format(perm_count=sum(len(v) for v in all_constraints.values()))
...
if __name__ == "__main__":
    ...
    row_constraints = load_all_row_constraints(base_dir, sample_size=10000)
    load_time = time.time() - start_load
    print(f"⏱ 讀取完成，耗時 {load_time:.2f} 秒")
    total_perms = sum(len(v) for v in row_constraints.values())
    print(f"📊 總排列模式: {total_perms}")
    print("-" * 70)
    print()
    ...
    solver.generate_visualization(html_path, row_constraints)
