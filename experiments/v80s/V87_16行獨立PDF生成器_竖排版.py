...
# -*- coding: utf-8 -*-
"""
V87 16行獨立PDF生成器 - 竖排版修正
修正排版超出界麵的問題，採用豎排布局和縮小字體
"""
...
OUTPUT_DIR = 'D:\\Users\\Jualius\\WorkBuddy\\3d_sudoku_system\\output\\V87_16行獨立PDF_竖排'
...
def create_title_table(row_name, entropy_label):
    ...
    # 使用2列布局，減少單列寬度
    header_data = [
        [f'符闔排列256數獨', f'{row_name}行獨立測試題型'],
        [f'熵級: {entropy_label}', f'錨點: {DATA["anchor_analysis"]["by_row"][row_name]["initial"]} → 16'],
    ]
    
    # 可用寬度: 210mm - 36mm(邊距) = 174mm, 分2列 = 87mm/列
    t = Table(header_data, colWidths=[87*mm, 87*mm])
    t.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (0, -1), HEADER_BG),
        ('BACKGROUND', (1, 0), (1, -1), SUBTITLE_BG),
...
def create_puzzle_grid_vertical(puzzle_data, target_row=None):
    """創建豎排數獨網格 - 修正版：每4列一組，換行顯示"""
    rows = list('ABCDEFGHIJKLMNOP')
    story = []
    ...
    for group_idx in range(4):
        ...
        # 表頭
        header = ['行'] + [f'{i+1}' for i in range(group_start, group_start+4)] + ['錨點']
        table_data = [header]
        ...
        for row_idx, row_name in enumerate(rows):
            ...
            cell_values = []
            for val in row_data[group_start:group_start+4]:
                if val == 0:
                    cell_values.append('·')
                else:
        ...
        # 計算欄寬：(可用寬度174mm - 行標識12mm - 錨點列12mm) / 4 = 37.5mm
        col_widths = [12*mm] + [37.5*mm]*4 + [12*mm]
...
def create_comparison_table_vertical(target_row):
    """創建謎盤vs解盤對比表 - 豎排版（每4個位置一組）"""
    ...
    for group_idx in range(4):
        group_start = group_idx * 4
        positions = list(range(group_start, group_start+4))
        
        # 標題行
        header = ['類型'] + [f'Pos {i+1}' for i in positions]
        table_data = [header]
        
        # 謎盤行
        puzzle_row = ['謎盤']
        for i in positions:
            val = initial[target_row][i]
        ...
        solution_row = ['解盤']
        for i in positions:
            solution_row.append(str(final[target_row][i]))
        ...
        for pos in new_positions:
            if group_start <= pos < group_start + 4:
                col_in_group = pos - group_start
                x = col_in_group + 1
...
def create_stats_table(target_row):
    ...
    stats_data = [
        ...
        [f'新增錨點位置', str([p+1 for p in info['new_anchor_positions']])],
    ]
...
def generate_row_pdf(row_name):
    ...
    output_file = f'{OUTPUT_DIR}/V87_{row_name}行獨立測試題型_竖排.pdf'
    ...
    puzzle_grid = create_puzzle_grid_vertical(DATA['initial_puzzle'], target_row=row_name)
    for item in puzzle_grid:
        story.append(item)
    story.append(Spacer(1, 6*mm))
    ...
    solution_grid = create_puzzle_grid_vertical(DATA['final_solution'], target_row=row_name)
    for item in solution_grid:
        story.append(item)
    story.append(Spacer(1, 6*mm))
    ...
    comparison = create_comparison_table_vertical(row_name)
